...
int main(void) {
    ...
    //SetupAlarm_Demo();
    SetupScheduleTable_Demo();      // Ở đây chạy xong có thể lệch 1,2.. hay bao nhiêu ms nữa của counter vì systick kích hoạt trước rồi mà
    StartScheduleTableRel(0, 50);      // Thì sau cái 1,2.. ms đó ở đây đếm thêm 50ms nữa thì  mới kích hoạt table schedule của mình (tức tại thời điểm 51 hay 52 hay 5.. ms nữa mới kích hoạt table schedule)

                    // Lỗi lần đầu chạy start_time đã = 1 không phải 51, vì nó setup đến dòng 254 có khả nằng hàm ngắt systick xảy ra 1 lần rồi và nó setup giá trị rác vô 
                        // table schedule.
